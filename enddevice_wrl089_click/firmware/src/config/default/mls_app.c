...
StackRetStatus_t MlsAppSendReq(void)
{
    ...
    {
        uint8_t appLEDs;
        uint8_t appLED1 = LED_BLUE_Get()  ; // record the application BLUE_LED state
        uint8_t appLED2 = LED_GREEN_Get() ; // record the application GREEN_LED state
        
        appLEDs = (appLED1 << 1) + appLED2;
                
        TEMPSENSOR_Read((uint8_t *)&celsius);
        fahrenheit = MlsAppCelsiusToFahrenheit(celsius);
        snprintf((char *)mlsAppBuf, sizeof(mlsAppBuf), "%d/%.1fC/%.1fF",appLEDs, celsius, fahrenheit);
    ...
    switch (data->evt)
    {
        ...
        case LORAWAN_EVT_RX_DATA_AVAILABLE:
        {
            ...
            if ((data->param.rxData.dataLength > 0) && (data->param.rxData.pData))
            {
                ...
                //todo
                switch (data->param.rxData.pData[1])        //itt tartok
					{
						case 0x30:
							LED_BLUE_Clear() ;
							LED_GREEN_Clear() ;                    
